...
test.describe("SpartanAI E2E Test Suite", () => {
  ...
  test("Standard Login Flow", async ({ page }) => {
    ...
    await emailInput.fill(process.env.TEST_EMAIL || "admin@spartanai.local");
  ...
  test("Master Mode and WebAuthn UI", async ({ page }) => {
    const masterButton = page
      .locator("div")
      .filter({ hasText: "Shield" })
      .nth(1); // Click shield icon to toggle master mode
    ...
    const shieldToggle = page
      ...
      .locator("xpath=..")
      .locator("div")
      .first(); // Or just the element with class containing cursor-pointer
  ...
  test("Dashboard and Threat Alerts", async ({ page }) => {
    ...
    await expect(
      page.locator("button", { hasText: "OP_ENCLAVE" }),
    ).toBeVisible();
    await expect(
      page.locator("button", { hasText: "MSF_FRAMEWORK" }),
    ).toBeVisible();
