...
async function loginAndWaitForDashboard(page: import("@playwright/test").Page) {
  ...
  await page
    .locator('input[placeholder="OPERATOR_ID (Username)"]')
    .fill(process.env.TEST_EMAIL || "admin@spartanai.local");
  await page
    .locator('input[type="password"]')
    .fill(process.env.TEST_PASSWORD || "password123");
...
test.describe("SpartanAI Security Suite - Comprehensive Tests", () => {
  test.describe("Authentication", () => {
    test("Login page renders correctly", async ({ page }) => {
      ...
      await expect(
        page.locator("text=Enterprise Security Intelligence Suite"),
      ).toBeVisible();

      // Verify form fields exist
      await expect(
        page.locator('input[placeholder="OPERATOR_ID (Username)"]'),
      ).toBeVisible();
      await expect(page.locator('input[type="password"]')).toBeVisible();

      // Verify login button
      await expect(
        page.locator("button", { hasText: "INITIALIZE SECURE ACCESS" }),
      ).toBeVisible();
    ...
    test("Successful login navigates to dashboard", async ({ page }) => {
      ...
      await expect(
        page.locator("text=SPARTANAI_SECURITY_CORE_SECURITY_CONSOLE"),
      ).toBeVisible();
    });
    ...
    test("Failed login shows error message", async ({ page }) => {
      ...
      await page
        .locator("button", { hasText: "INITIALIZE SECURE ACCESS" })
        .click();

      // Verify error message appears
      await expect(
        page.locator("text=/Authentication failed|Access denied/i"),
    ...
    test("Master Mode toggle shows WebAuthn UI", async ({ page }) => {
      ...
      await expect(
        page.locator("text=AUTHORIZE SOVEREIGN ENTRY"),
      ).toBeVisible();
      await expect(
        page.locator("button", { hasText: "Register Key" }),
      ).toBeVisible();
      await expect(
        page.locator("button", { hasText: "Authenticate Key" }),
      ).toBeVisible();
      ...
      await expect(emailInput).toHaveValue(
        process.env.TEST_EMAIL || "admin@spartanai.local",
      );
    });
  ...
  test.describe("Dashboard", () => {
    ...
    test("Holographic Control Hub is displayed with toggles", async ({
      page,
    }) => {
    ...
    test("SpartanAI_Security_Core Shell is displayed and accepts input", async ({
      page,
    }) => {
      await expect(
        page.locator("text=SpartanAI_Security_Core Shell"),
      ).toBeVisible();

      // Verify terminal has initial messages
      await expect(
        page.locator("text=SPARTANAI_SECURITY_CORE_SHELL v2.5.0-STABLE READY."),
      ).toBeVisible();
    ...
    test('Terminal executes "status" command', async ({ page }) => {
      ...
      await expect(
        page.locator("text=operator@spartanai-security-core:~$ status"),
      ).toBeVisible({ timeout: 10000 });
    });
    ...
    test('Terminal executes "probe network" command', async ({ page }) => {
      ...
      await expect(
        page.locator("text=NETWORK RECON PROBE INITIATED"),
      ).toBeVisible({ timeout: 10000 });
    });
  ...
  test.describe("Sidebar Navigation", () => {
    ...
    test("Clicking OP_ENCLAVE tab switches view", async ({ page }) => {
      ...
      await expect(page.locator("text=SYSTEM_STATUS_REPORT")).not.toBeVisible({
        timeout: 5000,
      });
    });
  ...
  test.describe("Settings Panel", () => {
    ...
    test("Settings panel opens and closes", async ({ page }) => {
      ...
      await expect(page.locator("text=SYSTEM_CONFIGURATION")).toBeVisible({
        timeout: 5000,
      });

      // Close settings with the Close button
      await page.locator("button", { hasText: "Close" }).click();
      await expect(page.locator("text=SYSTEM_CONFIGURATION")).not.toBeVisible({
        timeout: 5000,
      });
    });
  ...
  test.describe("Top Navigation Bar", () => {
    ...
    test("Operational state indicator is visible", async ({ page }) => {
      await expect(
        page.locator("text=OPERATIONAL STATE: ACTIVE // REGION: US-WEST"),
      ).toBeVisible();
    });
  ...
  test.describe("Footer Bar", () => {
    ...
    test("Footer shows system status and version", async ({ page }) => {
      ...
      await expect(
        page.locator(
          "text=SpartanAI Security Core Intelligence v2.5.0-Production",
        ),
      ).toBeVisible();
    });
    ...
    test("Footer shows sync status", async ({ page }) => {
      await expect(
        page.locator("text=SECURE_SYNC: AUTHENTICATED"),
      ).toBeVisible();
    });
  });
});
