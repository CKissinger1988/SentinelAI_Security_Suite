...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [token, setToken] = useState<string | null>(
    localStorage.getItem("spartanai_security_core_jwt_token"),
  );
  ...
  const isSecOpsAdmin = () =>
    user?.role === "SecOps-Admin" || user?.role === "root";
  const isAuditor = () =>
    ["root", "SecOps-Admin", "Compliance-Auditor"].includes(user?.role || "");
  const isHelpdesk = () => !!user;
  const canWrite = () => ["root", "SecOps-Admin"].includes(user?.role || "");
  const canAudit = () =>
    ["root", "SecOps-Admin", "Compliance-Auditor"].includes(user?.role || "");
  const canScan = () => !!user;
  const roleAtLeastFn = (min: UserRole) =>
    !!user && roleAtLeast(user.role, min);
