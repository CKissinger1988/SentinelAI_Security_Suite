  ...
  useEffect(() => {
    const init = async () => {
      ...
      for (let i = 0; i <= 100; i += 20) {
        ...
        if (i === 40)
          setMessages((prev) => [...prev, "NETWORK PROTOCOLS STABILIZED"]);
        if (i === 60)
          setMessages((prev) => [...prev, "BINARY INTEGRITY SECURE"]);
        if (i === 80)
          setMessages((prev) => [...prev, "NEURAL INTERFACE SYNCED"]);
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#02040a] flex items-center justify-center overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 space-y-8 relative">
        ...
        >
          ...
          <div className="space-y-1">
            <h1 className="text-xl font-bold tracking-[0.3em] text-white uppercase italic">
              SPARTANAI_SECURITY_CORE_OS
            </h1>
            <p className="text-[10px] font-mono text-cyan-500/50 uppercase tracking-widest">
              Initialization & Configuration Protocol
            </p>
          </div>
        </motion.div>
              ...
              {progress < 100 && (
                <div className="flex items-center gap-3 text-[9px] font-mono text-cyan-500/50 animate-pulse">
                  ...
                  <span className="uppercase tracking-tight">
                    Pulling latest security definitions...
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className="flex flex-col items-center gap-2">
              <div
                className={`p-2 rounded bg-slate-900 border ${progress > (i + 1) * 25 ? "border-emerald-500/50 text-emerald-500" : "border-slate-800 text-slate-600"}`}
              >
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none opacity-20">
        ...
        <div
          className="absolute bottom-10 right-10 w-96 h-96 border border-cyan-500/10 rounded-full animate-reverse-spin"
