...
import { Users, Plus, Trash2, Shield, Eye, Wrench, Crown, AlertTriangle, RefreshCw } from 'lucide-react';
import { useAuth, UserRole, ROLE_DISPLAY, ROLE_COLOR } from '../contexts/AuthContext';
...
const ROLE_CAPABILITIES: Record<UserRole, string> = {
  ...
  'SecOps-Admin': 'Full tactical write — patch deploy, config changes, threat ops.',
  'Compliance-Auditor': 'Read-only + audit report generation. Zero write access.',
...
export const UserManagement: React.FC = () => {
  ...
  const [newRole, setNewRole] = useState<Exclude<UserRole, 'root'>>('Helpdesk-Operator');
  ...
  if (!isCreator()) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4 text-center">
        ...
        <p className="text-red-400 font-mono text-sm uppercase tracking-widest">SOVEREIGN ACCESS REQUIRED</p>
        <p className="text-slate-600 text-xs font-mono">This panel is restricted to the Creator (root) tier.</p>
      </div>
    );
  }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 12) {
      setFormError('Password must be at least 12 characters (NIST SP 800-63B).');
      return;
    }
    setCreating(true);
    try {
  ...
  const handleDelete = async (uid: string) => {
    try {
      const res = await authenticatedFetch(`/api/admin/users/${uid}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-white uppercase italic">USER_REGISTRY</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {(['SecOps-Admin', 'Compliance-Auditor', 'Helpdesk-Operator'] as Exclude<UserRole, 'root'>[]).map(role => (
          <div
            key={role}
            className="p-3 rounded-xl border"
            style={{ borderColor: `${ROLE_COLOR[role]}30`, backgroundColor: `${ROLE_COLOR[role]}08` }}
          >
            ...
            <p className="text-[9px] text-slate-500 font-mono leading-relaxed">{ROLE_CAPABILITIES[role]}</p>
          </div>
        ))}
      </div>
      ...
      <AnimatePresence>
        {formVisible && (
          ...
          >
            <h3 className="text-xs font-bold text-cyan-400 uppercase tracking-widest">Provision New Operator</h3>
            {formError && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="text-[9px] text-slate-500 uppercase tracking-widest font-mono block mb-1">Email Address</label>
                <input
                  type="email"
                  required
                  value={newEmail}
              ...
              <div>
                <label className="text-[9px] text-slate-500 uppercase tracking-widest font-mono block mb-1">Display Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[9px] text-slate-500 uppercase tracking-widest font-mono block mb-1">
                  Password <span className="text-slate-600">(min 12 chars)</span>
                </label>
                <input
                  type="password"
                  required
              ...
              <div>
                <label className="text-[9px] text-slate-500 uppercase tracking-widest font-mono block mb-1">Authority Tier</label>
                <select
                  value={newRole}
                  onChange={e => setNewRole(e.target.value as Exclude<UserRole, 'root'>)}
                ...
                >
                  <option value="SecOps-Admin" className="text-amber-400 bg-black">🛡️ SecOps Admin (Full Write)</option>
                  <option value="Compliance-Auditor" className="text-cyan-400 bg-black">👁️ Compliance Auditor (Read-Only)</option>
                  <option value="Helpdesk-Operator" className="text-green-400 bg-black">🛠️ Helpdesk Operator (Scan Only)</option>
                </select>
              </div>
            </div>
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ backgroundColor: `${ROLE_COLOR[u.role as UserRole]}15`, color: ROLE_COLOR[u.role as UserRole] }}
                  >
                    {ROLE_ICON[u.role as UserRole]}
                  </div>
                  <div>
                    <div className="text-sm font-mono text-white">{u.displayName}</div>
                ...
                <div className="flex items-center gap-4">
                  <span
                    ...
                    style={{ color: ROLE_COLOR[u.role as UserRole], backgroundColor: `${ROLE_COLOR[u.role as UserRole]}15` }}
                  >
                    {ROLE_DISPLAY[u.role as UserRole]}
                  </span>
                  {deleteConfirm === u.uid ? (
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] text-red-400 font-mono">Confirm?</span>
                      <button
