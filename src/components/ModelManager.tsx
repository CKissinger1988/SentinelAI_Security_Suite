...
export const ModelManager: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<"installed" | "discovery">(
    "installed",
  );
    ...
    <div className="space-y-8">
              ...
              .map((model) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div
                      className={`p-3 rounded-xl ${model.active ? "bg-cyan-500/20 text-cyan-400" : "bg-slate-800/50 text-slate-600"}`}
                    >
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[8px] font-mono bg-slate-800 px-1.5 py-0.5 rounded text-slate-500">
                          v{model.version}
                        </span>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {model.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[8px] font-bold uppercase tracking-widest text-slate-500 px-2 py-1 rounded bg-slate-900/50 border border-slate-800/50"
                        >
                          {tag}
                        </span>
                      ))}
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {registryModels.map((rm) => (
                <div
                  key={rm.id}
                  className="p-6 bg-slate-900/20 border border-slate-800 rounded-2xl flex items-center justify-between group hover:border-indigo-500/30 transition-all"
                >
                  <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[8px] bg-indigo-500/20 text-indigo-400 px-1.5 py-0.5 rounded font-mono uppercase tracking-widest">
                          {rm.provider}
                        </span>
                      </div>
                      <div className="flex gap-2">
                        {rm.tags.map((t) => (
                          <span
                            key={t}
                            className="text-[9px] font-mono text-slate-600 bg-black/40 px-2 py-0.5 rounded flex items-center gap-1.5"
                          >
              ...
              {registryModels.length === 0 && !loading && (
                <div className="col-span-full py-20 text-center space-y-4">
                  ...
                  <p className="text-slate-600 font-mono text-xs uppercase tracking-widest italic">
                    Syncing with global registry... No models found for current
                    filter.
                  </p>
                </div>
              )}
      ...
      <div className="mt-8 p-6 bg-slate-900/20 border border-slate-800 rounded-2xl flex flex-col items-center justify-center text-center space-y-4">
        ...
        <div className="max-w-md">
          <h4 className="text-slate-500 font-bold uppercase tracking-widest text-[9px]">
            Isolation Protocols
          </h4>
          <p className="text-[10px] text-slate-600 font-mono mt-1 italic leading-relaxed">
            All neural artifacts are sandboxed using hardware-level isolation.
            Discovery layer is restricted to verified repositories only.
            Handshake protocols use mutual TLS with hardware-based keys.
          </p>
        </div>
      </div>
    </div>
