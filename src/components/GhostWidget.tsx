...
export const GhostWidget: React.FC<GhostWidgetProps> = ({ stealthActive }) => {
  ...
  const [bandwidthNoise, setBandwidthNoise] = useState<number[]>(
    Array(10).fill(5),
  );
  ...
  return (
    <div className="glass-hologram p-5 rounded-2xl flex flex-col justify-between min-h-[220px] relative overflow-hidden transition-all duration-300">
      <div className="absolute top-0 right-0 p-3 opacity-15">
        <Ghost
          className={`w-16 h-16 ${stealthActive ? "text-cyan-500 animate-pulse" : "text-slate-500"}`}
        />
      </div>
      ...
      <div className="flex items-center justify-between mb-4 z-10">
        <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
          <Ghost
            className={`w-4 h-4 ${stealthActive ? "text-cyan-400 animate-bounce" : "text-slate-500"}`}
          />
          Ghost Operations Center
        </h3>
        <span
          className={`text-[8px] font-mono uppercase px-2 py-0.5 rounded-full border ${
            stealthActive
        ...
        <div className="flex items-center gap-2 font-mono text-[9px]">
          {hops.map((hop, i) => (
            <React.Fragment key={i}>
              ...
              {i < hops.length - 1 && (
                <span
                  className={`text-[10px] ${stealthActive ? "text-cyan-500 animate-pulse" : "text-red-500"}`}
                >
                  &rarr;
                </span>
              )}
      ...
      <div className="grid grid-cols-2 gap-4 mt-2 z-10">
        <div className="p-2.5 bg-black/40 border border-white/5 rounded-xl space-y-1">
          ...
          <p className="text-[9px] text-purple-400 font-bold font-mono truncate">
            {secondaryKey}
          </p>
        </div>
