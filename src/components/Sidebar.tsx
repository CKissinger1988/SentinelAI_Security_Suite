  ...
  return (
    <div className="w-20 lg:w-48 h-full bg-[#02040a]/80 border-r border-slate-800/50 flex flex-col transition-all duration-300 backdrop-blur-md overflow-y-auto overflow-x-hidden hide-scrollbar">
      <div className="p-4 px-6 flex items-center gap-3">
        ...
        <span className="hidden lg:block font-bold text-lg tracking-tighter text-white">
          SPARTANAI_SECURITY_CORE
        </span>
      </div>
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <Icon
                className={`w-5 h-5 ${isActive ? "text-cyan-500" : "group-hover:scale-110 transition-transform"}`}
              />
              <span className="hidden lg:block font-medium text-[10px] tracking-widest uppercase">
                {tab.label}
              </span>
            </button>
          );
      ...
      <div className="p-4 px-6 border-t border-slate-800/30 space-y-3">
        ...
        {user && (
          <div
            className="hidden lg:block px-3 py-2 rounded-xl border mb-2"
            style={{
              borderColor: `${ROLE_COLOR[user.role as UserRole]}30`,
              backgroundColor: `${ROLE_COLOR[user.role as UserRole]}08`,
            }}
          >
            <div
        ...
        {hsmOnline && (
          <div className="hidden lg:flex items-center gap-3 px-3 py-2 bg-emerald-500/5 border border-emerald-500/20 rounded-xl mb-4">
            ...
            <div className="flex flex-col">
              <span className="text-[8px] font-bold text-emerald-400 uppercase tracking-tighter">
                HSM Locked
              </span>
              <span className="text-[7px] text-emerald-500/50 font-mono">
                FIPS-140-2
              </span>
            </div>
          </div>
