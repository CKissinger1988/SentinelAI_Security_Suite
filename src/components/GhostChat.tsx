...
export const GhostChat: React.FC = () => {
  ...
  >([
    {
      id: "1",
      text: "SECURE_CHANNEL_ESTABLISHED. ENCRYPTION: AES-256-GCM. JARVIS_NEURAL_LINK: STANDBY.",
      sender: "system",
    },
  ]);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3 scrollbar-thin scrollbar-thumb-slate-800">
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`max-w-[85%] rounded px-2.5 py-1.5 text-[10px] font-mono ${
                      msg.sender === "user"
            ...
            <form
              onSubmit={handleSend}
              className="p-2 border-t border-cyan-900/50 bg-black/60 flex items-center gap-2 shrink-0"
            >
              <input
                type="text"
                value={input}
              ...
              <button
                type="submit"
                disabled={!input.trim()}
                className="text-cyan-600 hover:text-cyan-400 disabled:opacity-50 transition-colors p-1"
              >
      ...
      >
        <div
          className="absolute inset-0 bg-cyan-500/10 rounded-full animate-ping"
