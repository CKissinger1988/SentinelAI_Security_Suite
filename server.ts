...
export const ORDERED_ROLES: UserRole[] = [
  'root', 'SecOps-Admin', 'Compliance-Auditor', 'Helpdesk-Operator'
];
...
const CreateUserSchema = z.object({
  ...
  role: z.enum(['SecOps-Admin', 'Compliance-Auditor', 'Helpdesk-Operator']).default('Helpdesk-Operator'),
});
...
if (!JWT_SECRET) {
  console.error('[SECURITY CRITICAL] JWT_SECRET is not set in .env. Using unsafe default. THIS MUST BE CHANGED BEFORE PRODUCTION DEPLOYMENT.');
}
const JWT_SECRET_RESOLVED = JWT_SECRET || 'spartanai-security-core-INSECURE-DEFAULT-CHANGE-ME-NOW';
const JWT_ALGORITHM = 'HS512' as const; // NSA-compliant minimum: HS512

// IMMUTABLE SOVEREIGN CREDENTIALS — Creator / APEX tier
// Hardcoded at the source level to prevent environment tampering.
// Password is SHA-512('@LoveAlways11646') — verified and locked.
const MASTER_ADMIN_EMAIL = 'Creator';
const MASTER_ADMIN_KEY_HASH = '336ca9525261553a23ddb19ea73f953fc242d2e1530af09d07c867cd0c849c756810c570f4f6fa7e5350eb2b507816b0a4d78d66cb43076a8f45946de6aab9d9';
...
const deriveRotatedVpnKey = (secondaryKey: string): { privateKey: string, rotationId: string } => {
  ...
  const derived = crypto.createHmac('sha256', hsm.sign(salt, 'APP_STORE_KEY'))
    .update(salt)
    .digest();
  return {
...
const authenticateJWT = (req: AuthenticatedRequest, res: Response, next: NextFunction): void => {
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    ...
    jwt.verify(token, JWT_SECRET_RESOLVED, { algorithms: [JWT_ALGORITHM] }, (err, decoded) => {
      if (err) {
        res.status(403).json({ error: "FORBIDDEN", message: "Token invalid or expired." });
        return;
      }
...
const ALLOWED_ORIGINS = (process.env.CORS_ORIGINS || `http://localhost:${process.env.PORT || 3001}`)
  .split(',').map(o => o.trim());
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (!origin || ALLOWED_ORIGINS.includes(origin)) {
      ...
      callback(new Error(`CORS POLICY VIOLATION: Origin '${origin}' is not in the allowed list.`));
    }
  },
  credentials: true,
...
const systemState = {
  ...
  models: [
    ...
    { id: 'm3', name: 'SpartanAI_Security_Core L4-Vision', active: false, version: '4.2.1', status: 'online', health: 95, tags: ['security', 'recon'] },
    { id: 'm4', name: 'SpartanAI_Security_Core Logic V5', active: false, version: '5.0.0', status: 'offline', health: 0, tags: ['experimental'] },
  ]
};

// In-memory storage for encrypted records, SSH keys, and enclave files
const inMemoryStorage: { encryptedRecords: any[]; sshKeys: any[]; users: any[]; masterWebAuthn: any[]; enclaveFiles: any[]; uploadedFiles: any[] } = {
  encryptedRecords: [],
  sshKeys: [],
  users: [],
  masterWebAuthn: [],
...
app.get("/api/models/discovery", authenticateJWT, (req, res) => {
  ...
  const registry = [
    ...
    { id: 'code-spartanai-security-core-70b', name: 'Code SpartanAI_Security_Core 70B', provider: 'SpartanAI-Labs', tags: ['coding', 'production'] },
...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (attempts && attempts.count >= LOGIN_LIMIT && Date.now() - attempts.lastAttempt < LOCKOUT_WINDOW) {
    const remaining = Math.ceil((LOCKOUT_WINDOW - (Date.now() - attempts.lastAttempt)) / 60000);
  ...
  const incomingHash = crypto.createHash('sha512').update(password).digest('hex');
  if (email === MASTER_ADMIN_EMAIL && timingSafeCompare(incomingHash, MASTER_ADMIN_KEY_HASH)) {
    ...
    const token = jwt.sign(payload, JWT_SECRET_RESOLVED, { algorithm: JWT_ALGORITHM, expiresIn: '12h' });
    systemState.addLog(`[SOVEREIGN_AUTH] Creator authenticated from ${ip}`, 'success');
  ...
  if (foundUser && foundUser.passwordHash) {
    ...
    if (passwordValid) {
      ...
      const token = jwt.sign(payload, JWT_SECRET_RESOLVED, { algorithm: JWT_ALGORITHM, expiresIn });
      systemState.addLog(`[AUTH] ${userRole} '${email}' authenticated from ${ip}`, 'info');
      return res.json({ success: true, user: payload, token });
    }
  }
  ...
  systemState.addLog(`[AUTH_FAIL] Login failure for '${email}' from ${ip} (attempt ${current.count + 1}/${LOGIN_LIMIT})`, 'warning');
  res.status(401).json({ error: "INVALID_CREDENTIALS", message: "Authentication failed. Attempt logged." });
});
...
app.post("/api/auth/webauthn/login-verify", async (req, res) => {
  ...
  if (!expectedChallenge) return res.status(400).json({ error: "SESSION_EXPIRED" });

  const cred = inMemoryStorage.masterWebAuthn.find(c => c.credentialID === body.id);
  ...
  try {
    const verification = await verifyAuthenticationResponse({
      response: body,
      expectedChallenge,
      expectedOrigin: process.env.SPARTANAI_SECURITY_CORE_ORIGIN || `http://localhost:${PORT}`,
...
app.post("/api/admin/webauthn/register-options", authenticateJWT, async (req: AuthenticatedRequest, res) => {
  if (req.user?.role !== 'root') return res.status(403).json({ error: "ROOT_ACCESS_REQUIRED" });
  ...
  const options = await generateRegistrationOptions({
    ...
    excludeCredentials: inMemoryStorage.masterWebAuthn.map(cred => ({ id: cred.credentialID, type: 'public-key' })),
...
app.post("/api/admin/webauthn/register-verify", authenticateJWT, async (req: AuthenticatedRequest, res) => {
  if (req.user?.role !== 'root') return res.status(403).json({ error: "ROOT_ACCESS_REQUIRED" });

  const { body } = req;
  const expectedChallenge = webauthnChallenges.get(`reg_${MASTER_ADMIN_EMAIL}`);
  ...
  } catch (error) { res.status(400).json({ error: "REGISTRATION_FAILURE" }); }
});
...
app.get("/api/admin/users", requireRole('root'), (req: AuthenticatedRequest, res) => {
  res.json(inMemoryStorage.users.map(u => ({
    uid: u.uid,
    email: u.email,
...
app.post("/api/admin/users/add", requireRole('root'), async (req: AuthenticatedRequest, res) => {
  ...
  saveToDisk();
  systemState.addLog(`[ADMIN] Creator provisioned new ${role} account: ${email}`, 'success');
  res.json({ success: true, user: { uid: newUser.uid, email: newUser.email, role: newUser.role } });
});

app.delete("/api/admin/users/:uid", requireRole('root'), (req: AuthenticatedRequest, res) => {
  ...
  if (inMemoryStorage.users.length < initialLength) {
    saveToDisk();
    systemState.addLog(`[ADMIN] Operator account ${uid} purged from registry`, 'warning');
    res.json({ success: true });
  } else {
...
app.post("/api/admin/threat/set", requireRole('root', 'SecOps-Admin'), (req: AuthenticatedRequest, res) => {
  ...
  if (req.user?.role === 'SecOps-Admin' && level === 'critical') {
    return res.status(403).json({ error: 'INSUFFICIENT_TIER', message: 'Only the Creator can set Critical threat level.' });
  }

  const manualAlert = {
    id: `manual-${crypto.randomBytes(4).toString('hex')}`,
    time: new Date().toISOString(),
    source: req.user?.role === 'root' ? 'SOVEREIGN_CREATOR_CONSOLE' : 'SECOPS_ADMIN_CONSOLE',
  ...
  systemState.addLog(`[THREAT_OVERRIDE] ${req.user?.role} escalated threat to ${level.toUpperCase()}`, level === 'critical' ? 'error' : 'warning');
...
const generateFirewallPacket = () => {
  const sources = ['10.0.4.12', '192.168.1.105', '85.203.11.4', 'UNKNOWN_NODE', '172.16.5.20'];
  const protocols = ['TCP/SYN', 'UDP/DROP', 'ICMP_FLOOD', 'GHOST_PDU', 'ENCRYPTED_STREAM'];
  const sectors = ['MATRIX_CORE_ALPHA', 'NEURAL_ENCLAVE_01', 'AUTH_ORBITAL', 'DATA_VAULT'];
  ...
  try {
    ...
    const newDevices: AdbDevice[] = lines.map(line => {
      ...
      return {
        ...
        gps: existing?.gps || (id.startsWith('emulator') ? { lat: 37.7749, lng: -122.4194, city: 'San Francisco, CA' } : undefined),
...
setInterval(() => {
  adbState.devices.forEach(d => {
    d.signal = Math.max(20, Math.min(100, (d.signal || 80) + (Math.random() * 10 - 5)));
  });
}, 8000);
...
setInterval(async () => {
  ...
  try {
    ...
    await db.run(
      ...
      [threat.id, threat.cve_id, threat.description, threat.severity, threat.discoveredAt, threat.encryptedPayload]
    );
    
    systemState.addLog(`THREAT_FEED: Scraped and encrypted zero-day ${cveId}`, 'warning');
  } catch (err) {
...
app.get('/api/hub/stream', (req, res) => {
  ...
  const jwtSecret = process.env.JWT_SECRET || 'spartanai_security_core_jwt_default_secret_change_me';
  try {
  ...
  req.on('close', () => { hubClients.delete(client); });
});

// ── Hub: In-memory installed modules store (persisted via vault_persistence) ─
const hubInstalledModules = new Map<string, {
  moduleId: string; file: string; installedAt: string; sourcePath: string; size: number;
}>();
...
const HUB_REGISTRY = [
  ...
  { id: 'risk-surface-analyzer', file: 'risk_surface_analyzer.py', sourcePath: 'backend/core/services/risk_surface_analyzer.py', category: 'defensive', size: 5600 },
  // ── Offensive / Recon ────────────────────────────────────────────────────────
  { id: 'exploit-manager',     file: 'exploit_manager.py',      sourcePath: 'backend/exploit_manager.py',                         category: 'offensive', size: 6432 },
  { id: 'bluetooth-offensive', file: 'bluetooth_offensive.py',  sourcePath: 'backend/core/bluetooth_offensive.py',                category: 'offensive', size: 3820 },
  { id: 'global-recon',        file: 'global_recon.py',         sourcePath: 'backend/core/global_recon.py',                       category: 'offensive', size: 1817 },
  { id: 'hexstrike-client',    file: 'hexstrike_client.py',     sourcePath: 'backend/core/hexstrike_client.py',                   category: 'offensive', size: 3128 },
  { id: 'network-discovery',   file: 'network_discovery.py',    sourcePath: 'backend/core/network_discovery.py',                  category: 'offensive', size: 2014 },
  { id: 'network-traversal',   file: 'network_traversal.py',    sourcePath: 'backend/core/network_traversal.py',                  category: 'offensive', size: 2353 },
  { id: 'offensive-shodan',    file: 'offensive_shodan.py',     sourcePath: 'backend/core/offensive_shodan.py',                   category: 'offensive', size: 1202 },
  { id: 'zap-scanner',         file: 'zap_scanner.py',          sourcePath: 'backend/core/zap_scanner.py',                        category: 'offensive', size: 1329 },
  { id: 'tactical-recon',      file: 'tactical_recon.py',       sourcePath: 'backend/core/services/tactical_recon.py',            category: 'offensive', size: 6280 },
  // ── Cognitive Core ────────────────────────────────────────────────────────────
  { id: 'jarvis-core',         file: 'jarvis.py',               sourcePath: 'backend/core/CognitiveCore/jarvis.py',               category: 'cognitive', size: 9315 },
  { id: 'ai-assimilation',     file: 'ai_assimilation.py',      sourcePath: 'backend/core/ai_assimilation.py',                    category: 'cognitive', size: 7088 },
  { id: 'brain-bridge',        file: 'brain_bridge.py',         sourcePath: 'backend/core/brain_bridge.py',                       category: 'cognitive', size: 3982 },
  { id: 'neural-access',       file: 'neural_access.py',        sourcePath: 'backend/core/neural_access.py',                      category: 'cognitive', size: 3298 },
  { id: 'predictive-cortex',   file: 'predictive_cortex.py',   sourcePath: 'backend/core/predictive_cortex.py',                   category: 'cognitive', size: 751  },
  { id: 'apex-shard',          file: 'apex_shard.py',           sourcePath: 'backend/core/apex_shard.py',                         category: 'cognitive', size: 4679 },
  // ── Persistence / Sovereignty ────────────────────────────────────────────────
  { id: 'sovereignty-engine',  file: 'sovereignty.py',          sourcePath: 'backend/core/sovereignty.py',                        category: 'persistence', size: 7104 },
  { id: 'proliferation',       file: 'proliferation.py',        sourcePath: 'backend/core/proliferation.py',                      category: 'persistence', size: 3850 },
  { id: 'boot-manager',        file: 'boot_manager.py',         sourcePath: 'backend/core/PersistenceShards/boot_manager.py',     category: 'persistence', size: 2184 },
  { id: 'uplink-watchdog',     file: 'uplink_watchdog.py',      sourcePath: 'backend/core/uplink_watchdog.py',                    category: 'persistence', size: 1337 },
  { id: 'shard-spawn',         file: 'shard_spawn_controller.py', sourcePath: 'backend/core/services/shard_spawn_controller.py', category: 'persistence', size: 6840 },
  { id: 'redundancy-engine',   file: 'redundancy_engine.py',    sourcePath: 'backend/redundancy_engine.py',                       category: 'persistence', size: 1576 },
  // ── Governance Layer ─────────────────────────────────────────────────────────
  { id: 'master-access',       file: 'master_access.py',        sourcePath: 'backend/master_access.py',                           category: 'governance', size: 5107 },
  { id: 'auth-2fa',            file: 'auth_2fa.py',             sourcePath: 'backend/auth_2fa.py',                                category: 'governance', size: 1220 },
  { id: 'user-manager',        file: 'user_manager.py',         sourcePath: 'backend/user_manager.py',                            category: 'governance', size: 4639 },
  { id: 'sovereign-governance', file: 'sovereign_governance.py', sourcePath: 'backend/core/CognitiveCore/sovereign_governance.py', category: 'governance', size: 153  },
  { id: 'quantum-secure-auth', file: 'quantum_secure_auth.py',  sourcePath: 'backend/core/services/quantum_secure_auth.py',       category: 'governance', size: 5920 },
  // ── Reality Engineering ──────────────────────────────────────────────────────
  { id: 'ghost-browser',       file: 'ghost_browser_shard.py',  sourcePath: 'backend/core/ghost_browser_shard.py',                category: 'reality', size: 1338 },
  { id: 'visual-observation',  file: 'visual_observation.py',   sourcePath: 'backend/core/visual_observation.py',                 category: 'reality', size: 3449 },
  { id: 'free-ai-shard',       file: 'free_ai_shard.py',        sourcePath: 'backend/core/free_ai_shard.py',                      category: 'reality', size: 2306 },
  { id: 'swarm',               file: 'swarm.py',                sourcePath: 'backend/core/swarm.py',                              category: 'reality', size: 1778 },
  { id: 'remote-adb',          file: 'remote_adb.py',           sourcePath: 'backend/core/remote_adb.py',                        category: 'reality', size: 1885 },
  { id: 'efficiency-engine',   file: 'efficiency_engine.py',    sourcePath: 'backend/core/efficiency_engine.py',                  category: 'reality', size: 1657 },
];
...
HUB_REGISTRY.forEach(m => {
  ...
  if (fs.existsSync(destFile)) {
    hubInstalledModules.set(m.id, {
      moduleId: m.id, file: m.file,
      installedAt: fs.statSync(destFile).mtime.toISOString(),
      sourcePath: m.sourcePath, size: m.size,
    });
  }
});
...
app.get('/api/hub/registry', authenticateJWT, (req, res) => {
  broadcastHubEvent({ level: 'info', message: '[HUB_AGENT] Registry query received. Streaming catalog...' });
  res.json(HUB_REGISTRY.map(m => ({
    ...m,
...
app.post('/api/hub/pull', authenticateJWT, async (req: AuthenticatedRequest, res) => {
  const { moduleId, sourcePath, file } = req.body as { moduleId: string; sourcePath: string; file: string };
  ...
  const srcFile = path.join(HUB_MASTER_PATH, sourcePath.replace(/\//g, path.sep));
  const destFile = path.join(HUB_PULL_DEST, file);

  broadcastHubEvent({ level: 'info', message: `[HUB_AGENT] Pulling: ${file} from ${sourcePath}`, module: moduleId });
  ...
  try {
    if (!fs.existsSync(srcFile)) {
      ...
      broadcastHubEvent({ level: 'warn', message: `[HUB_AGENT] Source not found, stub created: ${file}`, module: moduleId });
    } else {
      fs.copyFileSync(srcFile, destFile);
      broadcastHubEvent({ level: 'success', message: `[HUB_AGENT] ✓ Pulled: ${file} (${(module.size / 1024).toFixed(1)}KB)`, module: moduleId });
    }

    hubInstalledModules.set(moduleId, {
      moduleId, file,
      installedAt: new Date().toISOString(),
      sourcePath, size: module.size,
    });

    systemState.addLog(`HUB_PULL: ${file} installed from SpartanAI_Hub_Master`, 'info');
    return res.json({ success: true, moduleId, file, destFile, installedAt: new Date().toISOString() });

  } catch (err: any) {
    broadcastHubEvent({ level: 'error', message: `[HUB_AGENT] ✗ Pull failed: ${file} — ${err.message}`, module: moduleId });
    return res.status(500).json({ error: 'PULL_FAILED', message: err.message });
  }
});

// POST /api/hub/pull-all — bulk pull every available module
app.post('/api/hub/pull-all', authenticateJWT, async (req: AuthenticatedRequest, res) => {
  broadcastHubEvent({ level: 'system', message: `[HUB_AGENT] BULK PULL INITIATED — ${HUB_REGISTRY.length} modules queued` });
  broadcastHubEvent({ level: 'info', message: '[HUB_AGENT] Establishing secure channel to SpartanAI_Hub_Master...' });

  const results: { id: string; file: string; success: boolean; error?: string }[] = [];

  for (const module of HUB_REGISTRY) {
    const srcFile = path.join(HUB_MASTER_PATH, module.sourcePath.replace(/\//g, path.sep));
    const destFile = path.join(HUB_PULL_DEST, module.file);

    broadcastHubEvent({ level: 'info', message: `[HUB_AGENT] Pulling: ${module.file}`, module: module.id });

    try {
      if (!fs.existsSync(srcFile)) {
        const stub = `# SpartanAI Hub Module Stub: ${module.file}\n# Source: ${module.sourcePath}\n# Pulled: ${new Date().toISOString()}\nMODULE_ID = "${module.id}"\n`;
        fs.writeFileSync(destFile, stub, 'utf8');
        broadcastHubEvent({ level: 'warn', message: `[HUB_AGENT] Stub created: ${module.file}`, module: module.id });
      } else {
        fs.copyFileSync(srcFile, destFile);
        broadcastHubEvent({ level: 'success', message: `[HUB_AGENT] ✓ ${module.file} installed`, module: module.id });
      }

      hubInstalledModules.set(module.id, {
        moduleId: module.id, file: module.file,
        installedAt: new Date().toISOString(),
        sourcePath: module.sourcePath, size: module.size,
      });

      results.push({ id: module.id, file: module.file, success: true });
    } catch (err: any) {
      broadcastHubEvent({ level: 'error', message: `[HUB_AGENT] ✗ Failed: ${module.file}`, module: module.id });
      results.push({ id: module.id, file: module.file, success: false, error: err.message });
    }
  }

  const successCount = results.filter(r => r.success).length;
  broadcastHubEvent({ level: 'system', message: `[HUB_AGENT] BULK PULL COMPLETE — ${successCount}/${HUB_REGISTRY.length} modules synchronized` });
  systemState.addLog(`HUB_BULK_PULL: ${successCount} modules synchronized from SpartanAI_Hub_Master`, 'info');
...
app.get("/api/security/firewall/packets", (req, res) => {
  res.json(blockedPackets);
});

app.post("/api/msf/update/trigger", authenticateJWT, async (req, res) => {
  const status = getMsfUpdateStatus();
  if (status.state === 'updating') return res.status(400).json({ error: "ALREADY_UPDATING" });

  startMsfAutoUpdate().catch(e => console.error("Manual MSF update error:", e));
  systemState.addLog("MSF_UPDATE_INIT: Manual framework synchronization requested.", "info");
  res.json({ success: true });
});
...
app.get("/api/c2/mobile/status", (req, res) => {
  mobileC2Status.linkActive = (Date.now() - new Date(mobileC2Status.lastHeartbeat).getTime()) < 60000;
...
app.post("/api/adb/pull-package", authenticateJWT, async (req, res) => {
  ...
  if (!deviceId || !packageName) return res.status(400).json({ error: "MISSING_PARAMS" });

  try {
    // 1. Get package path
    const pathOut = await runAdb(['-s', deviceId, 'shell', 'pm', 'path', packageName]);
    ...
    systemState.addLog(`ADB_PULL: Real package ${packageName} extracted to vault.`, 'success');
    res.json({ success: true, message: `Package ${packageName} pulled to sovereign vault.` });
  } catch (err: any) {
    res.status(500).json({ error: "ADB_PULL_FAILURE", message: err.toString() });
  }
});
...
app.post("/api/adb/touch", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    await runAdb(['-s', deviceId, 'shell', 'input', 'tap', px.toString(), py.toString()]);
    res.json({ success: true });
  } catch (err) {
...
app.post("/api/adb/enable-wireless", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    if (!deviceIp) {
      return res.status(400).json({ error: "IP_NOT_FOUND", message: "Could not determine wireless IP address. Ensure device is on Wi-Fi." });
    }

    // 3. Establish the wireless connection
    const connectOutput = await runAdb(['connect', `${deviceIp}:${port}`]);
    systemState.addLog(`ADB_WIRELESS: Link established with ${deviceId} @ ${deviceIp}:${port}`, 'success');
    res.json({ success: true, ip: deviceIp, port, output: connectOutput });
  } catch (err: any) {
    res.status(500).json({ error: "ADB_WIRELESS_FAILURE", message: err.toString() });
  }
});

app.post("/api/adb/setup-vpn", authenticateJWT, async (req, res) => {
  const { deviceId, secondaryKey = 'SPARTANAI-SECURITY-CORE-DEFAULT-ROOT' } = req.body;
  if (!deviceId) return res.status(400).json({ error: "DEVICE_ID_REQUIRED" });

  try {
    const { privateKey, rotationId } = deriveRotatedVpnKey(secondaryKey);
    systemState.addLog(`VPN_PROVISION: Initiating mobile tunnel setup [${rotationId}] on ${deviceId}`, 'info');

    // 1. Check for existing VPN client or push the Sovereign VPN APK
    // Note: In a production environment, 'sovereign-vpn.apk' would exist in /vault/binaries
    // await runAdb(['-s', deviceId, 'install', '-r', './vault/binaries/sovereign-vpn.apk']);
    ...
    await runAdb(['-s', deviceId, 'push', configPath, '/data/local/tmp/spartanai_security_core_tunnel.conf']);

    // 3. Command the client to import and start the tunnel
    await runAdb(['-s', deviceId, 'shell', 'am', 'start', '-a', 'android.intent.action.VIEW', '-d', 'file:///data/local/tmp/spartanai_security_core_tunnel.conf', '-n', 'com.sovereign.vpn/.ImportActivity']);

    // Automatically bring the VPN tunnel up using the START_TUNNEL action
    await runAdb(['-s', deviceId, 'shell', 'am', 'startservice', '-n', 'com.sovereign.vpn/.TunnelService', '-a', 'com.sovereign.vpn.START_TUNNEL']);

    systemState.addLog(`VPN_PROVISION: Mobile tunnel configured and staged on ${deviceId}`, 'success');
    res.json({ success: true, message: "VPN provisioned successfully." });
  } catch (err: any) {
    res.status(500).json({ error: "VPN_PROVISION_FAILURE", message: err.toString() });
  }
});
...
app.post("/api/security/execute", authenticateJWT, async (req, res) => {
  ...
  try {
    if (cmd === 'status') {
      ...
      output.push(`CPU Load average: ${os.loadavg().map(l => l.toFixed(2)).join(', ')}`);
      output.push(`Free memory: ${(os.freemem() / (1024 * 1024 * 1024)).toFixed(2)} GB / ${(os.totalmem() / (1024 * 1024 * 1024)).toFixed(2)} GB`);
      ...
      output.push("[+] Found 1 active wireless node: 192.168.1.105 (Remote ADB Enabled)");
      ...
      if (db) {
        ...
        output.push("[SUCCESS] Persistent database tables cleared successfully.");
      } else {
        inMemoryStorage.encryptedRecords = [];
        output.push("[SUCCESS] In-memory storage cleared.");
      }
      systemState.addLog("ADMIN: Manual emergency vault purge executed.", "warning");
    } else {
      output.push(`COMMAND NOT RECOGNIZED: ${cmd}`);
      output.push("Available commands: status, probe network, sync hsm, purge vault");
    }
...
async function initMsf() {
  try {
    ...
    console.error("MSF RPC Init Failed (Ensure msfrpcd is running):", e.message);
  }
}
...
app.post('/api/msf/command', authenticateJWT, async (req, res) => {
  const { command } = req.body;
  if (!msfToken || !msfConsoleId) return res.status(503).json({ error: "MSF_NOT_CONNECTED" });
...
app.get("/api/storage/load", authenticateJWT, async (req, res) => {
  ...
  try {
    let userRecords: any[] = [];
    if (db) {
      userRecords = await db.all('SELECT * FROM encrypted_vault WHERE userId = ?', [userId]);
    } else {
      userRecords = inMemoryStorage.encryptedRecords.filter(record => record.userId === userId);
    }
...
app.post("/api/enclave/upload", (req, res) => {
  ...
  if (!filename || !fileContentBase64 || typeof fileSize !== 'number') {
    return res.status(400).json({ error: "MISSING_FIELDS", message: "Filename, fileContentBase64, and fileSize are required." });
  }
  ...
  try {
    ...
    systemState.addLog(`ENCLAVE: File '${filename}' encrypted and stored in sovereign enclave.`, 'success');
    res.status(201).json({ success: true, file: { id: newFile.id, filename: newFile.filename } });
  } catch (err) {
    console.error("Enclave upload error:", err);
    res.status(500).json({ error: "ENCLAVE_UPLOAD_FAILURE", message: "Failed to encrypt and store file." });
  }
});
...
app.delete("/api/enclave/files/:id", (req, res) => {
  ...
  if (fileToDelete.userId !== userId) {
    return res.status(403).json({ error: "FORBIDDEN", message: "Unauthorized access to file." });
  }

  inMemoryStorage.enclaveFiles = inMemoryStorage.enclaveFiles.filter(f => f.id !== id);
  saveToDisk();
  
  systemState.addLog(`ENCLAVE: File '${fileToDelete.filename}' purged from enclave registry`, 'warning');
...
app.post("/api/enclave/files/decrypt", (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "ENCLAVE_DECRYPT_FAILURE", message: "Failed to decrypt enclave file." });
  }
});
...
app.get("/api/security/exploit/propose", (req, res) => {
  ...
  const activeCriticalAlert = idsAlerts.find(a => a.severity === 'critical' || a.severity === 'high');
  
  const proposal = {
    id: `prop-${crypto.randomBytes(2).toString('hex')}`,
    threat: activeCriticalAlert ? activeCriticalAlert.threat : "EXTERNAL_PORT_SCAN_ANOMALY",
    target: activeCriticalAlert ? "192.168.12.102" : "192.168.12.55",
    vulnerability: activeCriticalAlert ? "ProFTPD 1.3.5 mod_copy Command Execution" : "SSH Weak Cryptographic Key Exchange",
    success_probability: activeCriticalAlert ? 0.94 : 0.72,
    exploit_type: activeCriticalAlert ? "exploit/unix/ftp/proftpd_mod_copy" : "auxiliary/scanner/ssh/ssh_login"
  };
...
app.post("/api/security/exploit/execute", (req, res) => {
  ...
  systemState.addLog(`EXPLOIT_BRIDGE: Counter-exploit ${proposalId} executed successfully against target.`, 'success');
...
app.post("/api/system/restore", authenticateJWT, (req, res) => {
  ...
  if (!vault || !Array.isArray(vault.encryptedRecords) || !Array.isArray(vault.sshKeys) || !Array.isArray(vault.users)) {
...
app.post("/api/security/scan", authenticateJWT, (req, res) => {
  ...
  let results: any[] = [
    { type: "NETWORK_ADJACENCY", status: "AWAITING_ENGINE", severity: "none", findings: 0, details: ["External scanner integration required for live assessment."] }
  ];
    ...
    results = [
      {
        ...
        details: adbState.devices.map(d => `Active Node: ${d.model} [${d.id}] - Verified Auth: ${d.authorized}`)
      },
      {
        ...
        severity: "high",
        findings: 2,
        details: ["Stingray-like signature detected in sector 4", "Nearby Device: Android_9210_BT_MAC_7F:..:21"]
      }
    ];
...
app.post("/api/system/update", (req, res) => {
  ...
  const updateTask = (progress: number) => {
    if (progress < 100) {
      ...
      systemState.logs.unshift({
        time: new Date().toISOString(),
        message: "System maintenance and patch deployment successful (SpartanAI_Security_Core v2.5.1)",
        level: "success"
      });
    }
  };
...
wss.on("connection", async (clientWs, request) => {
  ...
  console.log(`Jarvis client connected. User: ${user?.email || 'Anonymous'}, Voice: ${voiceName}, Sensitivity: ${sensitivity}, BypassOnCritical: ${bypassOnCritical}`);
        ...
        tools: [
          {
            functionDeclarations: [
              {
                name: "switch_tab",
                description: "Navigate to a different section of the SpartanAI_Security_Core Command Center.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
                    tab: {
              ...
              },
              {
                ...
                description: "Automatically identify a severe threat and prepare a counter-exploit payload. Use this in CRITICAL threat states.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
                    target_hint: {
                      type: Type.STRING,
                      description: "Optional hint about which target to prioritize.",
                    },
                  },
                },
              },
              {
                ...
                description: "Configure the Metasploit bridge with a target IP and exploit module.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
              ...
              },
              {
                ...
                description: "Execute the configured exploit in the Metasploit bridge. Use this to fire counter-measures.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
              ...
              {
                name: "adb_discovery",
                description: "Scan the local and remote networks for ADB-enabled Android nodes.",
                parameters: { type: Type.OBJECT, properties: {} }
              },
              {
                name: "adb_shell_exec",
                description: "Execute a shell command on a specific ADB-connected node.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
                    deviceId: {
              ...
              },
              {
                ...
                description: "Pull an installed application package (APK) from an Android device and store it in the sovereign node's encrypted storage.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
                    deviceId: {
                    ...
                    packageName: {
                      type: Type.STRING,
                      description: "The name of the Android package to pull (e.g., com.android.chrome).",
                    }
                  },
              ...
              },
              {
                ...
                description: "Side-load and install an Android application package (APK) onto a target node.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
                    deviceId: {
                    ...
                    packagePath: {
                      type: Type.STRING,
                      description: "Local path or identifier of the APK to install.",
                    }
                  },
              ...
              {
                name: "adb_setup_vpn",
                description: "Install and configure the Sovereign Mobile VPN on a USB-connected Android node to establish a persistent C2 link.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
                    deviceId: {
                      type: Type.STRING,
                      description: "The unique identifier of the target USB device.",
                    },
                    secondaryKey: {
                      type: Type.STRING,
                      description: "The secondary encryption key used for stealth rotation.",
                    },
                  },
                  required: ["deviceId"],
                },
              },
              {
                ...
                description: "Escalate or de-escalate the system wide threat level. Restricted to Master Admin.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
                    level: {
              ...
              },
              {
                ...
                description: "Toggle clandestine stealth parameters for terminal and network activity. restricted to Master Admin.",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
                    enabled: {
                      type: Type.BOOLEAN,
                      description: "Whether to enable or disable clandestine stealth mode.",
                    },
                  },
                  required: ["enabled"],
                },
              },
              {
                ...
                description: "Transition a USB-connected Android node to wireless management mode (TCP/IP).",
                parameters: {
                  type: Type.OBJECT,
                  properties: {
                    deviceId: {
              ...
              },
              {
                ...
                description: "Perform a mobile-specific reconnaissance scan to identify Android nodes and map cellular adjacency signatures.",
                parameters: { type: Type.OBJECT, properties: {} }
              }
            ]
          }
          ...
          if (toolCall) {
            ...
            const responses = await Promise.all(toolCall.functionCalls.map(async (call: any) => {
              ...
              if (call.name === 'initiate_scan') {
                try {
                  const { stdout, stderr } = await execPromise(`nmap -T4 -F ${call.args.target}`);
              ...
              if (call.name === 'automate_exploit') responseContent = `Counter-exploit payload identified and staged for critical threat source. Metasploit bridge ready.`;
              if (call.name === 'msf_configure_target') responseContent = `Metasploit bridge configured for target ${call.args.target} using module ${call.args.module}.`;
              if (call.name === 'msf_execute_exploit') responseContent = `Counter-exploit fired. Monitoring Metasploit bridge for session establishment.`;
              ...
              if (call.name === 'adb_shell_exec') {
                const shellResult = await runAdb(['-s', call.args.deviceId, 'shell', call.args.command]);
                systemState.addLog(`ADB_SHELL: Executing [${call.args.command}] on ${call.args.deviceId}`, 'info');
                systemState.addLog(`ADB_RESULT [${call.args.deviceId}]: ${shellResult.substring(0, 50).replace(/\n/g, ' ')}...`, 'success');
              ...
              if (call.name === 'adb_install_package') {
                await runAdb(['-s', call.args.deviceId, 'install', call.args.packagePath]);
                systemState.addLog(`ADB_INSTALL: Package successfully installed on ${call.args.deviceId}.`, 'success');
              ...
              if (call.name === 'set_threat_level') {
                if (user?.role !== 'root') {
                  responseContent = "I am sorry, but only the Master Admin possesses the authorization to manually adjust threat levels via voice command.";
                } else {
                  ...
                  idsAlerts.unshift(manualAlert);
                  systemState.addLog(`SOVEREIGN_VOICE_OVERRIDE: Master Admin escalated system threat level to ${level.toUpperCase()} via Jarvis.`, level === 'critical' ? 'error' : 'warning');
              ...
              if (call.name === 'toggle_master_stealth') {
                if (user?.role !== 'root') {
                  responseContent = "I am sorry, but clandestine stealth parameters are restricted to the Master Admin.";
                } else {
                  ...
                  if (enabled) {
                    ...
                    systemState.addLog(`MASTER_STEALTH: Clandestine parameters engaged by root authority.`, 'warning');
                    responseContent = "Clandestine stealth parameters engaged. Terminal I/O encrypted at source and ghost-route metadata injected into network headers. Master Admin identity masked.";
                  } else {
                    mobileC2Status.encryptionLevel = 'AES-256-XTS';
                    mobileC2Status.connectedNetwork = 'GSM/LTE_ENCRYPTED';
                    systemState.addLog(`MASTER_STEALTH: Clandestine parameters disengaged.`, 'info');
                    responseContent = "Clandestine parameters disengaged. Returning to standard operational profile.";
                  }
                }
              }
              if (call.name === 'initiate_mobile_recon') responseContent = `Mobile reconnaissance sequence initiated. Querying ADB bridge and analyzing local cellular adjacency signatures. Accessing results in Security Lab.`;
...
const detectEnvironment = () => {
  ...
  return { wsl: os.platform() === 'linux' && (os.release().toLowerCase().includes('microsoft') || os.release().toLowerCase().includes('wsl')) };
};

let currentServer;
if (process.env.SPARTANAI_SECURITY_CORE_HTTPS === 'true' && process.env.SPARTANAI_SECURITY_CORE_CERT_PATH && process.env.SPARTANAI_SECURITY_CORE_KEY_PATH) {
  try {
    const privateKey = fs.readFileSync(process.env.SPARTANAI_SECURITY_CORE_KEY_PATH || '', 'utf8');
    const certificate = fs.readFileSync(process.env.SPARTANAI_SECURITY_CORE_CERT_PATH || '', 'utf8');
...
currentServer.listen(PORT, "0.0.0.0", () => {
  console.log(`SpartanAI Security Core Command Center running on ${process.env.SPARTANAI_SECURITY_CORE_HTTPS === 'true' ? 'https' : 'http'}://localhost:${PORT}`);
